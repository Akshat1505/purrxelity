...
def generate_message_plan(state:ReportState):
    prompt=ChatPromptTemplate.from_messages([
        ("system","You are a report planning agent. Your task is to generate a structured plan with atmost 6 sections for a given topic. The plan should be comprehensive and well-organized"),
        ("human","Generate a report plan for the topic: {input}")
    ])
    class Sections(BaseModel):
        sections:List[SectionState]=Field(description="List of sections for the report")
...
async def research_agent(state:ReportState):
    ...
    prompt=[
        ...
        HumanMessage(content=f"Research the following topic for the section '{section_to_research.name}': {section_to_research.description}"),
    ]
...
graph.add_edge("research_agent","research_tools")
graph.add_edge("research_tools","update_completed_sections")
